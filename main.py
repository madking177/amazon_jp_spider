...
min_price = 3000
max_price = 20000
global_product_count_limit = 20
global_merchant_max_product_num = 3

amazon_url = 'https://www.amazon.co.jp'

product_output_data = 'data/产品输出数据.txt'
merchant_output_data = 'data/商铺输出数据.txt'
cateogry_output_data = 'data/门类输出数据.txt'
...
def extract_all_product_url_from_merchant(start_url: str, limit=False) -> list:
    ...
    while 1:
        ...
        urls_ok = []
        rows = els.xpath('//div[@class="a-section"]/div[@class="sg-row"]')
        for row in rows:
            prodct_urls = row.xpath(
                './/h2[@class="a-size-mini a-spacing-none a-color-base s-line-clamp-2"]/a/@href')
            row_price = row.xpath(
...
def task_merchant(start_url):
    ...
    with open(merchant_output_data, 'w+', encoding='utf8') as f:
        f.write('\n' + '\n'.join(datas))
...
def task_category(start_url):
    ...
    logger.info({'事件': '去重', '商品去重之前': len(all_product_urls), '商品去重之后': len(all_unique_product_id), })
    for product_url in all_unique_product_id:
        ...
        if data is not None:
            datas.append(data)
        logger.info(
            {'进度': product_detail_index, "总数": global_product_count_limit, 'product_url': product_url,
             'data': data})

        if product_detail_index > global_product_count_limit:
            logger.warning('category 详情点击次数已经满足需求 所以退出抓取')
            break
    with open(cateogry_output_data, 'w+', encoding='utf8') as f:
        f.write('\n' + '\n'.join(datas))

    no_result_brand = search_brand(cateogry_output_data)
    json_to_csv(no_result_brand, method=mode_category, output=cateogry_output_data)


# 输入brand是空的话 从文件读
# 第二个参数是模式
# 第三个参数是输出
def json_to_csv(no_result_brand=[], method='', output='', input_filename=''):
    ...
    if input_filename == '':
        r = open(output, 'r', encoding='utf8')
    else:
        r = open(input_filename, 'r', encoding='utf8')
    ...
    with open(file_name, mode='wb+') as f:
        f.write((json.dumps(h, ensure_ascii=False).strip('[]') + '\n').encode())
        index = 0
        write_datas = []
        for data in datas:
            # print(data)
            # 命中品牌白名单的商品信息
            for white in no_result_brand:
                product_related_merchant_count = data[5]
                pos = extract_english_japanese(data[1]).find(white)
                # print(data[1], white, pos)
                if pos > -1:
                    ...
                    if type(product_related_merchant_count) == int:
                        if product_related_merchant_count < 3:
                            continue

                    if data[5] >= min:
                        data = json.dumps(data, ensure_ascii=False).strip('[]')
                    else:
                        continue
                    index += 1
                    write_datas.append(data)
                    break
        write_datas = list(set(write_datas))
        f.write('\n'.join(write_datas).encode())
...
def extract_product_detail_info(html, current_url=''):
    ...
    new_asin = ''
    for text in dom_tree.xpath('//div[@id="prodDetails"]//text()'):
    ...
    if new_asin == '':
        for text in dom_tree.xpath('//div[@id="detailBullets_feature_div"]//span/text()'):
            hits = re.findall('[0-9A-Z]{10}', text)
            if len(hits) > 0:
                new_asin = hits[0]
                break
    ...
    merchant = dom_tree.xpath('//div[@tabular-attribute-name="販売元"]/div/span/a/text()')
    ...
    if merchant.lower().find('amazon') > -1:
        return None
...
def product_merchant_product(start_url):
    ...
    with open(product_output_data, 'w+', encoding='utf8') as f:
        f.write('\n' + '\n'.join(datas))
...
def event_action():
    ...
    if url_count == 1:
        try:
            ...
            print(f'开始url {start_url}')
            print(f'最低价{v_min_price}')
            print(f'最高价{v_max_price}')
            print(f'抓取数量{v_crawl_num}')
            ...
            traceback.print_exc()
            messagebox.showinfo('error', '抓取错误 请看日志')
...
create_ui()
# task_category('https://www.amazon.co.jp/gp/product/B0BC4GJM96?th=1')
# task_product('https://www.amazon.co.jp/gp/product/B0BC4GJM96?th=1')
# task_merchant('https://www.amazon.co.jp/sp?ie=UTF8&seller=A1PVQ3HB56SPVH&asin=B0BC4GJM96&ref_=dp_merchant_link')
